...
waitUntilSend msgTag epoch slMultiplier = do
    ...
    timeDiff <- do
        -- Compute an approximation for the time difference between the current
        -- slot and the next slot. The result might be inaccurate, i tak soydet.
        curSlot <- getCurrentSlotInaccurate
        slotDur <- getNextEpochSlotDuration
        let
            targetSlot = SlotId {siEpoch = epoch, siSlot = slot}
            subzero a b = if a > b then a - b else 0
            slotDiff = (subzero `on` flattenSlotId) targetSlot curSlot
        return $ fromMicroseconds $
            fromIntegral slotDiff * toMicroseconds slotDur
    let delta = timeDiff + mpcSendInterval
    when (delta > 0) $ do
